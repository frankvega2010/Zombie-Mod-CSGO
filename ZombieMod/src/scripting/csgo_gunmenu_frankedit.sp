...
public Action:Command_GunMenu(client_index, args)
{
	...
	{
		client_team = Teams:GetClientTeam(client_index);
	}
	...
	if(GetClientCount(true))
		for(new client_index = 1; client_index <= MaxClients; ++client_index)
			if(IsClientInGame(client_index))
			{
				...
				CreateTimer(0.5, RoundShowMenu, GetClientUserId(client_index));
			}
	
}

public Action:RoundShowMenu(Handle:timer, any:user_index)
{
	new client_index = GetClientOfUserId(user_index);
	if(!client_index) return;
	if(IsFakeClient(client_index) && !g_AllowBots) return;
	
	if(g_PrimaryMenu != INVALID_HANDLE)DisplayMenu(g_PrimaryMenu, client_index, 6);
	else if(g_SecondaryMenu != INVALID_HANDLE) DisplayMenu(g_SecondaryMenu, client_index, 6);
}